...
#-----------------------------------------
    ...
    def createContours(self, image, lineThickness = 2):
        try:
            ...
            self.drawSVG(newContours, height, width, 100, 100, nameSVG, path, 2)      # draw it in the svg. Two 100s are the temporary number for resizing the svg. needs modify
            ...
            self.drawSVG(newContours, height, width, 100, 100, nameSVG2, path2, 2)    # draw it in the svg. Two 100s are the temporary number for resizing the svg. needs modify            
...
#-----------------------------------------
    ...
    def drawSVG(self, contourPoints, oreheight, orewidth, height, width, name = "contour_SVG", path = "./", mode = 1):
        try:
            ...
            percentx = width/orewidth
            percenty = height/oreheight
            ...
            for x in range(lengthOfTheList):
                ...
                x1 = math.floor(contourPoints[0][x][0] * percentx)           #resize x1
                x2 = math.floor(contourPoints[0][x+1][0] * percentx)         #resize x2
                y1 = math.floor(contourPoints[0][x][1] * percenty)           #resize y1
                y2 = math.floor(contourPoints[0][x+1][1] * percenty)         #resize y2
